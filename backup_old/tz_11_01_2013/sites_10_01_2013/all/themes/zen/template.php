...
function zen_breadcrumb($variables) {
  ...
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {
    ...
    if (!empty($breadcrumb)) {
      ...
      //Adding User Dashboard in Breadcrumb
      $current_path = current_path();
      if(preg_match('/node/', $current_path) || preg_match('/jobs/', $current_path)) {
  	$node = node_load(arg(1));
  	if($node->type == "job")
  		$user_dashboard = $trailing_separator . l("Dashboard", 'user-dashboard');
      }	
      else {
  	$user_dashboard = "";
      }		

      return '<div class="breadcrumb">' . $heading . implode($breadcrumb_separator, $breadcrumb) . $user_dashboard . $trailing_separator . $title . '</div>';
    }
  }
