...
function active_button() {
	...
	//alert(difference);
	if(difference == 0 || (difference <= .05 && difference >= -0.05)){		
...
/* Put the bank fee amount */
function change_bank_fee()
{
	var alloc_counter = jQuery('#counter_alloc').val();
	alloc_counter_div = parseInt(alloc_counter) + 1;
	var bank_fee = jQuery('#bank_fee').val();
	
	if(bank_fee == "" || bank_fee == ".")
		bank_fee = 0;
	//alert(bank_fee)
	
	bank_fee = parseFloat(bank_fee)
	
		
	var each_div = bank_fee / alloc_counter_div;
	
	//alert(each_div)
	
	for(i = 0; i <= alloc_counter; i++ )
	{
		var each_alloc_total = jQuery('#total_alloc_hid_'+i).val();
		each_alloc_total = parseFloat(each_alloc_total);
		each_alloc_total_div = each_alloc_total - each_div;
		
		jQuery('#edit-total-alloc-'+i).val(each_alloc_total_div.toFixed(2));
	}
	
	var total_allocated = jQuery('#total_resource_alloc').val();
	var new_total_allocated = total_allocated - bank_fee;
	
	jQuery('#total_allocated').val(new_total_allocated.toFixed(2));
